    ...
    <div class="all">
        ...
        <div class="j-alup">
            <div class="j-alupload-div" v-for="(item,index) in content" :id = "index">
                ...
                <p class="j-alupload-p all-link" @click="go_to_bookinfo(item.ancient_book_id)" :title="item.standard_name">{{item.standard_name}}</p>
            </div>
        </div>
        <!--翻页组件-->
    ...
    export default{
        ...
        data(){
            return{
                ...
                delete_url: '/ancient_books/delete_ancient_books.action',
                title: '已上传古籍',
                total_page: 1,
                i: 0,
                content: [],
